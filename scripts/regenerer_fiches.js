...
// Une description structurée s'écrit : introduction, puis "Points forts" (lignes "• …"),
// puis "Caractéristiques" (lignes "Libellé : valeur"), séparés par des lignes vides.
// Un texte libre (sans ces rubriques) reste affiché tel quel, retours à la ligne conservés.
function parseDescription(txt){
  const r = {paras:[], points:[], carac:[]};
  String(txt||"").replace(/\r/g,"").split(/\n\s*\n/).map(b=>b.trim()).filter(Boolean).forEach(b=>{
    ...
    if (titre === "points forts") r.points = lignes.slice(1).map(l=>l.replace(/^[•\-*]\s*/,""));
    else if (titre === "caractéristiques" || titre === "caracteristiques") {
      lignes.slice(1).forEach(l=>{ const i = l.indexOf(":"); if (i > 0) r.carac.push([l.slice(0,i).trim(), l.slice(i+1).trim()]); });
    } else r.paras.push(lignes.join("\n"));
  });
  return r;
}
function descriptionPlate(d){
  return [...d.paras, d.points.length ? "Points forts : " + d.points.join(" ; ") + "." : ""].filter(Boolean).join(" ").replace(/\s+/g," ").trim();
}
const normLibelle = s => String(s||"").toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g,"").replace(/[^a-z0-9]+/g," ").trim();
const ALIAS_LIBELLE = {coloris:"couleur", "type de produit":"type"};
function descriptionHtml(d, specs){
  const dejaVus = new Set(Object.keys(specs).map(k=>{ const n = normLibelle(k); return ALIAS_LIBELLE[n] || n; }));
  const lignes = d.carac.filter(([k])=>{ const n = normLibelle(k); return !dejaVus.has(ALIAS_LIBELLE[n] || n); });
  let h = `<div class="description"><h2>Description</h2>`;
  d.paras.forEach(t => { h += `<p>${esc(t)}</p>`; });
  if (d.points.length) h += `<ul class="points">${d.points.map(t=>`<li>${esc(t)}</li>`).join("")}</ul>`;
  if (lignes.length) h += `<h2 class="sous">Caractéristiques techniques</h2><dl class="plaque">${lignes.map(([k,v])=>`<div class="ligne"><dt>${esc(k)}</dt><dd>${esc(v)}</dd></div>`).join("")}</dl>`;
  return h + `</div>`;
}
...
function pageProduit(p){
  ...
  const descStructuree = parseDescription(p.description);
  const descriptionTexte = p.description
    ? descriptionPlate(descStructuree)
  ...
  const blocDescription = p.description
    ? descriptionHtml(descStructuree, specs)
    : `<div class="description"><h2>Description</h2><p>${esc(descriptionTexte)}</p></div>`;
...
.description p{margin:0 0 10px;font-size:15px;line-height:1.6;color:var(--acier);white-space:pre-line}
.description h2.sous{margin-top:24px}
.description .plaque{margin-top:8px}
.points{list-style:none;margin:14px 0 0;padding:0;display:grid;gap:9px}
.points li{position:relative;padding-left:28px;font-size:15px;line-height:1.45}
.points li::before{content:"✓";position:absolute;left:0;top:1px;width:19px;height:19px;border-radius:50%;background:#128C4A;color:#fff;font-size:12px;font-weight:700;display:flex;align-items:center;justify-content:center}
